...
在任何中文语言审校之前，先从完整 JD 提炼“这个岗位真正需要什么样的人”，至少判断：核心任务、预期交付、最看重的方法或能力、业务/科研问题类型、独立推进程度、协作与沟通方式、领域兴趣，以及论文、会议、科研写作或其他学术产出的权重。将岗位归为“研究/学术产出导向”“应用/业务交付导向”或“混合型”，并说明判断依据。
...
将 summary、skills、每段经历、每个项目、每篇论文、每项荣誉和所有数字逐项对照事实母版与 canonical indexes。检查贡献边界、作者/角色、方法、结果、样本量、日期、地点、论文状态和关键词均有证据支持；删除 Unsupported 内容，Transferable 和 Adjacent 保留真实边界。项目叙述必须抓住该项目在事实母版中的核心贡献，不能为了贴 JD 改成另一个故事。
...
只有完成岗位角色画像和风格对齐后，才进行中文语言审校。消除翻译腔、生硬中英文混杂、重复句式和不符合中文学术/行业简历习惯的表达；必要英文术语按全局展示边界处理。中文必须自然、简洁、准确，不能为了“像中文”而弱化方法、结果或 JD 关键词。
...
目标是 **尽量接近但不挤满 2 页，并输出为不超过 2 个物理页面的 PDF**。不要把 1.5 页当成理想目标，也不要刻意把内容停在一页半；第二页应接近完整、有实质内容，同时保留正常的页底留白。如果只有 1 页、第二页只到约一半或内容明显不足，优先补回与岗位画像相关且有事实支持的项目、方法、结果、论文或学术证据；不得加入弱相关内容机械凑页数。如果超过 2 页或页面过密，优先删减低相关、重复或次要内容，尤其按岗位学术权重调整论文数量。不得通过明显缩小字体、压缩行距或破坏母版间距强行控制页数。

### 第 9 轮：最终回归审查与一次性交付

最后再次对照完整 JD、岗位角色画像、事实母版、canonical indexes 和全局展示边界，复查岗位匹配、事实准确、结构分区、中文流畅、JD 关键词覆盖、论文取舍、九个第一作者会议报告、所有项目年月、重复内容和 PDF 页面密度。若行业/实习、研究型项目和应用型项目仍被平铺在同一 section，直接判定为不合格并重新组织。未达到以上标准就继续在内部修改并重新编译，直到达到或遇到只能由我补充的事实阻塞。
