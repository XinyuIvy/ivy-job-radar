...
type Analysis = {
  ...
  sourceDiagnostics?: { templateFile: string; atomicFactsFile: string; atomicFactCount: number };
      ...
      <div style={{ maxWidth: 1320, margin: "0 auto" }}>
        <header style={{ display: "flex", justifyContent: "space-between", gap: 20, marginBottom: 24 }}>
          <div>
            ...
            <p style={{ maxWidth: 820, lineHeight: 1.65 }}>读取所选中英文 LaTeX 母版与 Stage 3 原子事实，区分直接证据、可迁移能力、相邻经验和无证据要求，并生成可审核的逐条修改草案。</p>
          </div>
        ...
        {loadingApplication ? <p>正在读取申请…</p> : <section className="cv-grid" style={{ display: "grid", gridTemplateColumns: "minmax(320px,.82fr) minmax(500px,1.18fr)", gap: 18 }}>
          <div style={{ display: "grid", gap: 14, alignContent: "start" }}>
            ...
            {analysis && <article style={cardStyle}>
              ...
              <p style={{ marginTop: -5, color: "#5c665f", fontSize: 12 }}>实际读取：{analysis.sourceDiagnostics?.templateFile} · {analysis.sourceDiagnostics?.atomicFactCount ?? 0} 条原子事实</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 8 }}>
                <Metric label="JD 要求" value={analysis.summary.required} />
                <Metric label="母版已覆盖" value={analysis.summary.covered} />
                <Metric label="事实支持缺口" value={analysis.summary.supportedGaps} />
                <Metric label="仅相邻经验" value={analysis.summary.adjacentGaps} />
                <Metric label="无证据要求" value={analysis.summary.unsupportedGaps} />
              </div>
              <h3>母版已覆盖</h3>
              <p style={{ lineHeight: 1.7 }}>{covered.map((item) => item.keyword).join("、") || "无"}</p>
              <h3>未覆盖要求</h3>
              {[...supported, ...adjacent, ...unsupported].map((item) => <div key={item.keyword} style={{ borderTop: "1px solid #ded9ca", padding: "12px 0" }}>
                <strong>{item.keyword}</strong>
                <span style={{ float: "right", fontSize: 12 }}>{item.status === "supported_gap" ? "可由事实支持" : item.status === "adjacent_gap" ? "仅相邻" : "无证据"}</span>
          ...
          <div style={{ display: "grid", gap: 14, alignContent: "start" }}>
            <article style={cardStyle}>
              ...
              {!analysis ? <p>完成左侧分析后，这里会显示项目身份对齐结果、事实级证据和修改草案。</p> : <>
                <section>
                  <h3>推荐优先使用的项目</h3>
                  {(analysis.projects ?? []).length === 0 ? <p>暂未识别到项目推荐。</p> : (analysis.projects ?? []).map((project, index) => <div key={project.projectId} style={summaryBlock}>
                    ...
                    <p>{project.alreadyInTemplate ? "项目身份已与当前母版对齐；只考虑排序或 bullet 调整。" : "当前母版未识别到该项目；仅在它比现有项目更强时考虑替换加入。"}</p>
                ...
                <section>
                  ...
                  {supported.length === 0 ? <p>无。</p> : supported.map((item) => <div key={item.keyword} style={summaryBlock}>
                    ...
                    {(item.supportEvidence ?? []).map((evidence) => <div key={`${item.keyword}-${evidence.factId}`} style={evidenceBlock}>
                ...
                <section>
                  <h3>只能作为相邻经验</h3>
                  {adjacent.length === 0 ? <p>无。</p> : adjacent.map((item) => <p key={item.keyword}><strong>{item.keyword}</strong>：不能写成直接做过，只能在人工判断后描述底层相邻经验。</p>)}
                </section>
                <section>
                  <h3>事实不足，不能写入</h3>
                  {unsupported.length === 0 ? <p>无。</p> : <p>{unsupported.map((item) => item.keyword).join("、")}</p>}
                </section>
              </>}
            </article>

            {analysis && <article style={cardStyle}>
              <h2 style={headingStyle}>3. 逐条修改草案与 LaTeX Diff</h2>
              <p style={{ lineHeight: 1.6, color: "#536158" }}>这些是证据优先的审阅草案，不会自动改写母版。中文草案优先复用事实母版中的中文事实；若没有可靠中文对应句，则保留英文原子事实，避免自动误译。</p>
              ...
              <p style={{ marginTop: 16, padding: 12, borderRadius: 12, background: "#eef3ed", lineHeight: 1.6 }}>当前已接入 Stage 3 原子事实检索和可追溯 diff；embedding、reranker、capability ontology 和 Stage 5 行业翻译仍未接入，因此这里不会冒充完整 Hybrid RAG。</p>
            </article>}
          </div>
        </section>}
      </div>
