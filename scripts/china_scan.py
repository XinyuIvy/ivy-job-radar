#!/usr/bin/env python3
"""Public China platform scanner with salary retained for diagnostics only."""
...
_IMPL_PATH = Path(__file__).with_name("_china_scan_impl.py")
_SPEC = importlib.util.spec_from_file_location("_ivy_china_scan_impl", _IMPL_PATH)
if _SPEC is None or _SPEC.loader is None:
    raise RuntimeError(f"Cannot load China scanner implementation: {_IMPL_PATH}")
...
if hasattr(_IMPL, "HARD_REJECTION_KEYS"):
    _IMPL.HARD_REJECTION_KEYS = tuple(
        key for key in _IMPL.HARD_REJECTION_KEYS if key != "salary_below_20k"
    )
...
if hasattr(_IMPL, "HARD_REJECTION_KEYS"):
    globals()["HARD_REJECTION_KEYS"] = _IMPL.HARD_REJECTION_KEYS
