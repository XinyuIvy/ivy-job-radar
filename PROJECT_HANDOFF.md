...
## 17. 中国多来源桌面一键扫描（2026-08-01）

用户要求从 BOSS 城市计划中移除苏州，并把截图中的其余中国来源合并到现有桌面双击入口。分支 `agent/china-one-click-all-sources` 实现：

- BOSS 城市从 8 个缩为 7 个：上海、北京、深圳、杭州、广州、南京、成都；8 个关键词保持不变，总组合从 64 降为 56；
- 新增 `local-collector/china_one_click.py`，同一次运行先执行 BOSS 登录态批量采集，再执行公开索引扫描；
- 公开索引覆盖猎聘、智联、前程无忧、拉勾、牛客、国聘、应届生和中国公司官网；拉勾查询已补齐；
- 两段扫描彼此隔离：BOSS 登录或验证码异常不会阻止其他来源，公开索引失败也不会抹掉 BOSS 已同步结果；
- 每段合格岗位都直接调用现有网站导入接口，最终报告保存为 `~/.ivy-job-radar/reports/china-all-latest.json`；
- 现有桌面文件 `一键扫描BOSS.command` 在重新执行 install 后会被覆盖为多来源入口，避免桌面出现两个重复按钮；
- 猎聘等受保护页面当前仍以公开索引发现为主，不应声称为登录态完整页面自动抓取；具体 JD 可继续用统一书签补录；
- 本地 Python 编译和 4 项针对性测试通过。合并后需在用户 Mac 上 pull、重新 install，并完成一次真实多来源生产验收。
