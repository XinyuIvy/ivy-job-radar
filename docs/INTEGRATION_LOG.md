...
| Automatic onboarding for newly added company-pool companies | career-ops / ATS discovery pattern | Integrated | The unique-company count is recalculated from `app/company-pool.json` every run. A new row uses its configured source URL, Aggregator-derived official ATS evidence, public web discovery, then a bounded archive fallback. Unresolved companies enter the auditable failure queue automatically. |
...
| iCIMS, Paylocity, Jobvite, Taleo, Oracle Recruiting Cloud, SAP SuccessFactors | Partial | Official portal detection and generic/schema.org enumeration run automatically. Their tenant-specific structured endpoints remain a fallback gap because branded configurations vary; Aggregator official URLs still feed the shared verification path. |
...
- Production evidence for the new company-wide scanner will exist only after the next scheduled or confirmed manual workflow run.
