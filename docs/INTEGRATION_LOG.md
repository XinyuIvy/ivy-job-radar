...
- The first production receipt exposed two discovery gaps now fixed in code: regional/legacy company labels did not match parent-company Aggregator records, and stale sample job URLs could hide a still-live ATS board. The next scheduled run must measure the resulting coverage improvement.
