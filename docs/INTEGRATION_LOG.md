...
The website navigation was consolidated again on 2026-07-31 and now has seven primary destinations. Data Quality remains merged into Job Verification: the submission form appears first, followed by one queue for user-submitted requests and automated quality-control work, then aggregate health, run, and failure monitoring. The standalone Contacts destination was removed; recruiters, hiring managers, alumni, referral contacts, and interviewers are now created, edited, and followed inside the associated application record, while dated follow-ups still feed the calendar. The standalone Interview Preparation destination was merged into Company Research. Company-specific interview experiences now appear inside the matching company record, and role-family experiences remain on the same page. Company Research also reads verified official website and ATS/careers entries from the same source catalog consumed by the website ATS scanner, combines those links with current jobs, applications, interviews, contacts, and notes, and labels companies without verified entries as awaiting automatic discovery rather than asking the user to populate hundreds of records manually. Each interview-experience record retains its original source link and now provides a fallback source search; sources that may require sign-in are labeled accordingly.
